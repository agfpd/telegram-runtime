// approvalFace — telegram-runtime's live subscriber to the daemon approval broker
// (docs/17, U2). It OBSERVES the single in-daemon queue and drives the Telegram card
// lifecycle: a new `approval-request` → render a card (U3 handler); an `approval-resolved`
// (from ANY face — Telegram button, CLI, tray, timeout) → edit that card to its outcome.
//
// Transport is the fleet SSE stream (docs/15 `GET /fleet/v1/events`), which tail-follows
// the daemon's durable logs; approval events arrive with `src:"approvals"`. Contract
// obligations honoured: unknown `ev` ignored; events are AT-LEAST-ONCE (dedup by id);
// the SNAPSHOT is the state and events are hints — so on every (re)connect we reconcile
// against `GET /fleet/v1/approvals` and a periodic safety-net re-reconcile closes any gap
// between a reconcile and the live stream. The SSE `approval-request` carries only a
// compact summary, so the full `content` (criterion #7) is fetched per-id via getApproval;
// a request resolved before that GET is a 404 = "already handled, skip".
//
// This module owns the EVENT PLUMBING + DEDUP/RECONCILE state machine (unit-tested here);
// the actual Telegram rendering is injected as `handlers` (wired in U3). It never makes an
// approval DECISION and never fails safe to deny — that invariant lives in the broker/hook;
// a face that misses an event simply doesn't show a card, and the request still times out
// to a safe deny in the daemon.
...
/** The daemon log line shape carried in an approval SSE event's `data` (docs/17 §4). */
...
/** What the face tells the Telegram layer (U3). Both are best-effort and async. */
...
export interface ApprovalFaceOptions {
  base: string
  handlers: ApprovalFaceHandlers
...
/**
 * The live approval face. `start()` runs the connect→reconcile→stream→reconnect loop until
 ...
 */
export class ApprovalFace {
  private readonly base: string
  private readonly handlers: ApprovalFaceHandlers
  ...
  constructor(opts: ApprovalFaceOptions) {
    this.base = opts.base
  ...
  /** Route ONE parsed approval `data` object to the handlers, with dedup. Pure enough to
   *  unit-test: feed it decoded event data, assert the handler calls. Non-approval src /
   *  unknown ev are ignored (docs/15 obligation 1). */
  async ingest(data: ApprovalEventData): Promise<void> {
    if (data.src !== 'approvals' || !data.id) return
  ...
  private async showRequest(id: string): Promise<void> {
    ...
    await Promise.resolve(this.handlers.onRequest(item)).catch(err => {
  ...
  start(): void {
    ...
    this.running = true
    this.log('approval.face.start', { base: this.base })
  ...
  private async loop(): Promise<void> {
    ...
    while (this.running) {
      try {
        ...
        this.log('approval.face.stream.error', { err: String(err), attempt })
      }
  ...
  private async connectOnce(): Promise<void> {
    ...
    // replay=0: we do NOT want historical lifecycle events; the approval snapshot below
    // recovers any request that predates this connection.
    ...
    this.log('approval.face.connected', { base: this.base })
    ...
    try {
      for (;;) {
        ...
        for (const ev of events) {
          ...
          try {
            data = JSON.parse(ev.data) as ApprovalEventData
          } catch {
